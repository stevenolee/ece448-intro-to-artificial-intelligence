...
class TabQPolicy(QPolicy):
    ...
    def qvals(self, states):
        ...
        qvals = []
        for s in states:
            ind = self.discretize(s)
            qvals.append(self.model[ind])
        return qvals
    ...
    def td_step(self, state, action, reward, next_state, done):
        ...
        before = self.model[cur][action]
        target = reward + (0 if done else self.gamma * max(self.model[next]))
        self.model[cur][action] = self.model[cur][action] + self.lr * (target - self.model[cur][action])
        return (target - before) ** 2
